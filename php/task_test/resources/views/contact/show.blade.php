            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('contact.edit', ['id' => $contact->id]) }}" method="GET">
                        @csrf
                        <input class="btn btn-primary" type="submit" name="btn_edit" value="変更する" />
                    </form>
...
<!--
/************************************************
削除ボタンを押してすぐにレコードが削除されるのは問題なので、
いったんJSで確認メッセージを表示します。
************************************************/
//-->
function deletePost(e) {
    'use strict';
    if (confirm('本当に削除してよろしいですか？ (id=' + e.dataset.id + ')')) {
